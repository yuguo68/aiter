...
def _get_config(
    ...
    if not hasattr(_get_config, "_config_dict") or not hasattr(
        ...
        fpath = f"{AITER_TRITON_CONFIGS_PATH}/gemm/{dev}-GEMM-A16WFP4{shuffle_filename_suffix}.json"
    ...
    if key not in _get_config._config_dict.keys():
        dev = arch_info.get_arch()
        fpath = f"{AITER_TRITON_CONFIGS_PATH}/gemm/{dev}-GEMM-A16WFP4{shuffle_filename_suffix}-N={N}-K={2*K}.json"
