...
from .gated_delta_rule_utils import IS_AMD, IS_NVIDIA_HOPPER, USE_CUDA_GRAPH, autotune_cache_kwargs, check_shared_mem
...
@triton.autotune(
    configs=[
        triton.Config({'BV': BV}, num_warps=num_warps, num_stages=num_stages)
        for num_warps in [2, 4]
        for num_stages in ([3, 2] if IS_AMD else ([4, 3, 2] if check_shared_mem('ampere') else [1]))
        for BV in [64, 32]
    ],
